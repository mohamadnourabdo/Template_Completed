...
document.addEventListener("click", function (e) {
    if (!(e.target.classList.contains("InnerUl1")||e.target.classList.contains("InnerUl2")||e.target.id==="InnerUl1"||e.target.id==="InnerUl2")) {
        InnerUl1Class.style.cssText = `
        display: none;
    `;
    InnerUl2Class.style.cssText = `
    ...
    if (!(e.target.classList.contains("ul-content")||e.target.classList.contains("button-bars")||e.target.id === "InnerUl1"||e.target.id === "InnerUl2")) {
        contentLink.style.cssText = `
        right: -100%;
    `;
        buttonbars.children[0].style.cssText = `
...
let ArrayImage = ["image/undraw_Speech_to_text_re_8mtf.png",
